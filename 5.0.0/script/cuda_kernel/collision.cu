...
void collide_particles(double* __restrict__ pos, double* __restrict__ vel, const double* props, int* __restrict__ types,
                       int* __restrict__ head, const int* nxt, const int N,
                       const double cell_size, const int nx, const int ny, const int nz, unsigned long long* __restrict__ collision_count)
{
    // 1. 연산의 시행 여부 결정
                    ...
                    if (j > i) {
                        ...
                        if (dist2 > 1e-20 && dist2 < rsum*rsum) {
                            ...
                            if (v_rel_n < 0.0) {
                                ...
                                /*if ((type_i == 1 && type_j == 1) || (type_i == 1 && type_j == 1)) {
                                    // reaction 1. 활성화 에너지 체크 (넘어야 반응) / A + A -> B + B
                                    if (fabs(v_rel_n) > 900.0){

                                        int old_i = atomicCAS(&types[i], type_i, 2);
                                        int old_j = atomicCAS(&types[j], type_j, 2);

                                        if (old_i == type_i && old_j == type_j) {
                                            // 두 입자 모두 성공적으로 변했을 때만 카운트
                                            atomicAdd(collision_count, 1ULL);
                                        }

                                    }
                                }else if ((type_i == 2 && type_j == 2) || (type_i == 2 && type_j == 2)) {
                                    // reaction 2. 활성화 에너지 체크 (넘어야 반응) / B + B -> A + A
                                    if (fabs(v_rel_n) > 1100.0){

                                        int old_i = atomicCAS(&types[i], type_i, 1);
                                        int old_j = atomicCAS(&types[j], type_j, 1);
                                ...
                                // 입자의 곂침에 대한 보정 수행
                                double overlap = rsum - dist; // 곂친 깊이 계산
