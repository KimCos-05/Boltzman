...
def main(sim_params, run_params, result_dir, count):
    ...
    cp.savez_compressed(os.path.join(result_dir, 'step','step_0.npz'), positions=cp.asnumpy(positions), velocities=cp.asnumpy(velocities)) #물성 결과(positions, velocities) 저장
    ...
    for step in range(steps): # step의 개수만큼 반복
        ...
        if (step + 1) % dlog == 0: # 로그 빈도 조절
            ...
            cp.savez_compressed(os.path.join(result_dir, 'step',f'step_{step+1}.npz'), positions=cp.asnumpy(positions), velocities=cp.asnumpy(velocities)) #물성 결과(positions, velocities) 저장
    ...
    cp.savez_compressed(os.path.join(result_dir, 'step',f'step_{step+1}.npz'), positions=cp.asnumpy(positions), velocities=cp.asnumpy(velocities))
