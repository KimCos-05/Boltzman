...
import cupy as cp
import logging
from particle_setup import setParticle, setParticleSpeed

# CUDA 스크립트 불러오기
with open('cuda_kernel/build_list.cu', 'r', encoding = 'utf-8') as f:
    build_list_kernel_src = f.read()

with open('cuda_kernel/collision.cu', 'r', encoding = 'utf-8') as f:
...
collision_kernel = cp.RawKernel(collision_kernel_src, 'collide_particles')
...
def main(num_side = 47, dt = 1e-15, total_time = 1e-13, count = 0, dlog = 100):
    os.makedirs('results', exist_ok=True)
    
    # 1. 파라미터 설정 (상온의 아르곤 조건 가정)
    sx, sy, sz = 6.7e-8, 6.7e-8, 6.7e-8  # 공간 크기 (단위: m)
    r = 1.88e-10                         # 아르곤 반지름 (단위: m)
    m = 6.63e-26                         # 아르곤 질량 (단위: kg)
    T = 431.0                            # 초기 속도 (v_rms, 단위: m/s, ~298K)
    
    dt = dt                              # 시간 간격 (단위: s, 1 펨토초)
    ...
    pos_cpu, vel_cpu, prop_cpu = setParticle(sx, sy, sz, num_side, r, m) # 입자의 Position 생성
    ...
    velocities = setParticleSpeed(velocities, properties, T) #입자에 속도 부여

    #초깃값 저장
    logging.info(f"Step 0/{steps} 완료") # 로그 출력
    cp.savez_compressed(f'results/run_{count}/step/step_0.npz', positions=cp.asnumpy(positions), velocities=cp.asnumpy(velocities)) #물성 결과(positions, velocities) 저장
    ...
    for step in range(steps): # step의 개수만큼 반복
        ...
        collision_kernel((blocks_per_grid,), (threads_per_block,),
                         (positions.ravel(), velocities.ravel(), properties.ravel(),
        ...
        if (step + 1) % dlog == 0: # 로그 빈도 조절
            ...
            if log_count < 20:
                ...
                if deltatime > avg_time_per_log * 1.5:
                    logging.warning(f'시뮬레이션 속도 저하 감지 | 평균 : {avg_time_per_log:.2f} , 현재 : {deltatime:.2f}')
            ...
            logging.info(f"Step {step+1}/{steps} ({progress:.1f}%)완료 | 소요시간 : {deltatime:.2f}초 / 평균대비 {deltatime - avg_time_per_log:.2f}초 | 예상 남은시간 : {time_str}") # 로그 출력
            cp.savez_compressed(f'results/run_{count}/step/step_{step+1}.npz', positions=cp.asnumpy(positions), velocities=cp.asnumpy(velocities)) #물성 결과(positions, velocities) 저장
    ...
    cp.savez_compressed(f'results/run_{count}/step/step_{step+1}.npz', positions=cp.asnumpy(positions), velocities=cp.asnumpy(velocities))
