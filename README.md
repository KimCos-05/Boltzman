...
* Python과 C++(`CUDA Kernel)을 사용하여 대규모 입자 연산을 실시간 처리합니다.
* 또한 Cell Linked-List 방식을 구현하여 충돌 감지 복잡도를 $O(N^2)$에서 $O(N)$으로 최적화했습니다.
* 아래는 CUDA 및 Linked List 방식의 구현 전과 후의 실제 성능 테스트 결과입니다.
* 최적화는 Gemini 2.5 Pro 모델의 도움을 받았습니다.

| 구분 | 최적화 전 | 최적화 후 |
| :--- | :--- | :--- |
| 입자 수 | 8,000개 | 50,653개 |
| time / 1 step | 90s | 0.0024s |

2. **물리적 정합성 검증**

* 저장된 각 step별 데이터를 사용하여 에너지 보존 법칙 및 운동량 보존 법칙 준수를 검증합니다.
    
3. **데이터 시각화** 
* 3D 입자 거동 애니메이션 및 속력 분포 히스토그램 분석 도구를 포함합니다.
* 각 입자별 농도의 변화를 시각화합니다.
